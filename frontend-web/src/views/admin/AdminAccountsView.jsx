...
export const AdminAccountsView = () => {
  ...
  const accounts = useQuery({
    queryKey: ['admin', 'accounts', { q, activo, page }],
    queryFn: () => adminService.listarCuentas({ q, activo, page }),
  });
  const disable = useMutation({
    mutationFn: (id) => adminService.deshabilitar(id),
    ...
    onError: (error) => setMessage(extraerErroresApi(error).formulario || 'No se pudo deshabilitar la cuenta.'),
  });

  return (
    <main className="mx-auto max-w-7xl px-5 py-7">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div><h1 className="text-2xl font-bold">Cuentas propietarias</h1><p className="mt-1 text-sm text-slate-600">Consulta el estado y bloquea el acceso cuando corresponda.</p></div>
        <div className="flex flex-wrap gap-3">
          <label className="text-sm font-semibold">Acceso<select className="mt-1 block h-11 border border-slate-300 bg-white px-3" value={activo} onChange={(event) => { setActivo(event.target.value); setPage(1); }}><option value="">Todos</option><option value="true">Habilitadas</option><option value="false">Deshabilitadas</option></select></label>
          <label className="text-sm font-semibold">Buscar<span className="mt-1 flex h-11 items-center border border-slate-300 bg-white px-3"><Search size={17} className="text-slate-400" /><input className="ml-2 w-56 outline-none" value={q} onChange={(event) => { setQ(event.target.value); setPage(1); }} placeholder="Nombre, cédula o correo" /></span></label>
        </div>
      </div>
